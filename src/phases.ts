...
export class TitlePhase extends Phase {
  ...
  loadSaveSlot(slotId: integer): void {
    ...
    this.scene.ui.setMode(Mode.MESSAGE);
    this.scene.gameData
      .loadSession(this.scene, slotId, slotId === -1 ? this.lastSessionData : null)
  ...
  initDailyRun(): void {
    this.scene.ui.setMode(Mode.SAVE_SLOT, SaveSlotUiMode.SAVE, (slotId: integer) => {
      ...
      const generateDaily = (seed: string) => {
        ...
        for (const starter of starters) {
          const starterProps = this.scene.gameData.getSpeciesDexAttrProps(
            starter.species,
            starter.dexAttr
          );
        ...
        const modifiers: Modifier[] = Array(3)
          .fill(null)
          .map(() =>
            modifierTypes.EXP_SHARE().withIdFromFunc(modifierTypes.EXP_SHARE).newModifier()
          )
          .concat(
            Array(3)
              .fill(null)
              .map(() =>
                modifierTypes
                  .GOLDEN_EXP_CHARM()
                  .withIdFromFunc(modifierTypes.GOLDEN_EXP_CHARM)
                  .newModifier()
              )
          )
    ...
    if (this.loaded) {
      const availablePartyMembers = this.scene
        .getParty()
        .filter((p) => p.isAllowedInBattle()).length;
      ...
      if (
        ...
        if (availablePartyMembers > minPartySize) {
          this.scene.pushPhase(
            new CheckSwitchPhase(this.scene, 0, this.scene.currentBattle.double)
          );
          if (this.scene.currentBattle.double) {
            this.scene.pushPhase(
              new CheckSwitchPhase(this.scene, 1, this.scene.currentBattle.double)
            );
          }
        }
      }
...
export class ReloadSessionPhase extends Phase {
  ...
  start(): void {
    ...
    (this.systemDataStr
      ...
      : this.scene.gameData.loadSystem()
    ).then(() => {
      if (delayElapsed) {
        this.end();
      } else {
...
export class BattlePhase extends Phase {
  ...
  showEnemyTrainer(trainerSlot: TrainerSlot = TrainerSlot.NONE): void {
    ...
    for (let i = 0; i < sprites.length; i++) {
      const visible =
        !trainerSlot || !i === (trainerSlot === TrainerSlot.TRAINER) || sprites.length < 2;
...
export abstract class PartyMemberPokemonPhase extends FieldPhase {
  ...
  constructor(scene: BattleScene, partyMemberIndex: integer, player: boolean) {
    ...
    this.fieldIndex =
      partyMemberIndex < this.scene.currentBattle.getBattlerCount() ? partyMemberIndex : -1;
    this.player = player;
  }
...
export class EncounterPhase extends BattlePhase {
      ...
      if (enemyPokemon.species.speciesId === Species.ETERNATUS) {
        if (
          ...
          (battle.battleSpec === BattleSpec.FINAL_BOSS ||
            this.scene.gameMode.isWaveFinal(battle.waveIndex))
        ) {
        ...
        for (const enemyPokemon of battle.enemyParty) {
          if (enemyPokemon.isBoss()) {
            enemyPokemon.setBoss(
              true,
              Math.ceil(
                enemyPokemon.bossSegments * (enemyPokemon.getSpeciesForm().baseTotal / totalBst)
              )
            );
        ...
        if (!this.loaded) {
          this.scene.gameData
            .saveAll(
              this.scene,
              true,
              battle.waveIndex % 10 === 1 || this.scene.lastSavePlayTime >= 300
            )
            .then((success) => {
  ...
  doEncounterCommon(showEncounterMessage: boolean = true) {
        ...
        const showDialogueAndSummon = () => {
          this.scene.ui.showDialogue(message, trainer.getName(TrainerSlot.NONE, true), null, () => {
            this.scene.charSprite
              .hide()
              .then(() => this.scene.hideFieldOverlay(250).then(() => doSummon()));
          });
        };
        if (
          this.scene.currentBattle.trainer.config.hasCharSprite &&
          !this.scene.ui.shouldSkipDialogue(message)
        ) {
          this.scene
    ...
    if (this.scene.currentBattle.battleType !== BattleType.TRAINER) {
      enemyField.map((p) =>
        this.scene.pushConditionalPhase(
          new PostSummonPhase(this.scene, p.getBattlerIndex()),
          () => {
            ...
            const requiredPokemonsOnField = Math.min(
              this.scene.getParty().filter((p) => !p.isFainted()).length,
              2
            );
      ...
      this.scene.ui.showText(
        ...
        () => {
          this.scene.ui.showDialogue(
            battleSpecDialogue[BattleSpec.FINAL_BOSS].encounter,
            enemy.species.name,
            null,
            () => {
...
export class NextEncounterPhase extends EncounterPhase {
  ...
  doEncounter(): void {
    ...
    this.scene.tweens.add({
      targets: [
        this.scene.arenaEnemy,
        this.scene.arenaNextEnemy,
        this.scene.currentBattle.trainer,
        enemyField,
        this.scene.lastEnemyTrainer
      ].flat(),
      x: "+=300",
      duration: 2000,
      onComplete: () => {
...
export class SelectBiomePhase extends BattlePhase {
  ...
  start() {
    ...
    if (
      (this.scene.gameMode.isClassic &&
        this.scene.gameMode.isWaveFinal(this.scene.currentBattle.waveIndex + 9)) ||
      (this.scene.gameMode.isDaily &&
      ...
      onComplete: () => {
        ...
        this.scene.tweens.add({
          targets: [
            ...
            this.scene.arenaPlayerTransition
          ],
          duration: 1000,
          delay: 1000,
          ease: "Sine.easeInOut",
  ...
  preSummon(): void {
    ...
    if (!partyMember.isAllowedInBattle()) {
      ...
      const legalIndex = party.findIndex(
        (p, i) => i > this.partyMemberIndex && p.isAllowedInBattle()
      );
      if (legalIndex === -1) {
      ...
      [party[this.partyMemberIndex], party[legalIndex]] = [
        party[legalIndex],
        party[this.partyMemberIndex]
      ];
      console.warn(
  ...
  summon(): void {
    ...
    if (this.fieldIndex === 1) {
      ...
      pokemon.setFieldPosition(
        !this.scene.currentBattle.double || availablePartyMembers === 1
...
export class SwitchSummonPhase extends SummonPhase {
  ...
  preSummon(): void {
    ...
    if (
      !this.doReturn ||
      (this.slotIndex !== -1 &&
        !(this.player ? this.scene.getParty() : this.scene.getEnemyParty())[this.slotIndex])
    ) {
      if (this.player) {
    ...
    if (!this.batonPass) {
      (this.player ? this.scene.getEnemyField() : this.scene.getPlayerField()).forEach(
        (enemyPokemon) => enemyPokemon.removeTagsBySourceId(pokemon.id)
      );
    }
  ...
  switchAndSummon() {
    ...
    if (this.batonPass && switchedPokemon) {
      (this.player ? this.scene.getEnemyField() : this.scene.getPlayerField()).forEach(
        (enemyPokemon) =>
          enemyPokemon.transferTagsBySourceId(this.lastPokemon.id, switchedPokemon.id)
      );
      if (
        !this.scene.findModifier(
          (m) => m instanceof SwitchEffectTransferModifier && m.pokemonId === switchedPokemon.id
        )
      ) {
...
export class ShowTrainerPhase extends BattlePhase {
  ...
  start() {
    ...
    this.scene.trainer.setTexture(
      `trainer_${this.scene.gameData.gender === PlayerGender.FEMALE ? "f" : "m"}_back`
    );
...
export class ToggleDoublePositionPhase extends BattlePhase {
  ...
  start() {
    ...
    if (playerPokemon) {
      playerPokemon
        .setFieldPosition(
          this.double && this.scene.getParty().filter((p) => p.isAllowedInBattle()).length > 1
            ? FieldPosition.LEFT
            : FieldPosition.CENTER,
          500
        )
  ...
  start() {
    ...
    this.scene.ui.showText(
      i18next.t("battle:switchQuestion", {
        pokemonName: this.useName ? pokemon.name : i18next.t("battle:pokemon")
      }),
      null,
      () => {
...
export class SummonMissingPhase extends SummonPhase {
  ...
  preSummon(): void {
    this.scene.ui.showText(
      i18next.t("battle:sendOutPokemon", { pokemonName: this.getPokemon().name })
    );
  ...
  start() {
    ...
    if (moveQueue.length) {
      ...
      if (!queuedMove.move) {
        ...
        if (
          moveIndex > -1 &&
          playerPokemon.getMoveset()[moveIndex].isUsable(playerPokemon, queuedMove.ignorePP)
        ) {
  ...
  handleCommand(command: Command, cursor: integer, ...args: any[]): boolean {
    ...
    case Command.FIGHT:
      ...
      if (
        cursor === -1 ||
          ...
          (useStruggle =
            cursor > -1 &&
            !playerPokemon.getMoveset().filter((m) => m.isUsable(playerPokemon)).length)
      ) {
        ...
        const moveTargets: MoveTargetSet =
            args.length < 3 ? getMoveTargets(playerPokemon, moveId) : args[2];
        if (!moveId) {
        ...
        const errorMessage =
            playerPokemon.summonData.disabledMove === move.moveId
              ? "battle:moveDisabled"
        ...
        this.scene.arena.biomeType === Biome.END &&
          (!this.scene.gameMode.isClassic ||
            (this.scene
              ...
              this.scene.gameData.getStarterCount((d) => !!d.caughtAttr) <
                Object.keys(speciesStarters).length - 1))
      ) {
        this.scene.ui.setMode(Mode.COMMAND, this.fieldIndex);
        this.scene.ui.setMode(Mode.MESSAGE);
        this.scene.ui.showText(
    ...
    case Command.RUN:
      ...
      if (!isSwitch && this.scene.arena.biomeType === Biome.END) {
        ...
        if (!batonPass) {
          enemyField.forEach((enemyPokemon) =>
            applyCheckTrappedAbAttrs(CheckTrappedAbAttr, enemyPokemon, trapped, playerPokemon)
          );
        }
        if (batonPass || (!trapTag && !trapped.value)) {
          ...
          if (
            trapTag.sourceMove === Moves.INGRAIN &&
              this.scene.getPokemonById(trapTag.sourceId).isOfType(Type.GHOST)
          ) {
            success = true;
            this.scene.currentBattle.turnCommands[this.fieldIndex] = isSwitch
              ? { command: Command.POKEMON, cursor: cursor, args: args }
          ...
          this.scene.ui.showText(
            i18next.t("battle:noEscapePokemon", {
              ...
              escapeVerb: isSwitch
                ? i18next.t("battle:escapeVerbSwitch")
                : i18next.t("battle:escapeVerbFlee")
            }),
            null,
            () => {
              this.scene.ui.showText(null, 0);
...
export class EnemyCommandPhase extends FieldPhase {
  ...
  start() {
    ...
    if (trainer && !enemyPokemon.getMoveQueue().length) {
      ...
      const trapped = new Utils.BooleanHolder(false);
      opponents.forEach((playerPokemon) =>
        applyCheckTrappedAbAttrs(CheckTrappedAbAttr, playerPokemon, trapped, enemyPokemon)
      );
      if (!trapTag && !trapped.value) {
        const partyMemberScores = trainer.getPartyMemberMatchupScores(
          enemyPokemon.trainerSlot,
          true
        );
        ...
        if (partyMemberScores.length) {
          ...
          const matchupScore =
            matchupScores.reduce((total, score) => (total += score), 0) / matchupScores.length;

          const sortedPartyMemberScores =
            trainer.getSortedPartyMemberMatchupScores(partyMemberScores);

          const switchMultiplier =
            1 - (battle.enemySwitchCounter ? Math.pow(0.1, 1 / battle.enemySwitchCounter) : 0);

          if (
            sortedPartyMemberScores[0][1] * switchMultiplier >=
            matchupScore * (trainer.config.isBoss ? 2 : 3)
          ) {
    ...
    this.scene.currentBattle.enemySwitchCounter = Math.max(
      this.scene.currentBattle.enemySwitchCounter - 1,
      0
    );
  ...
  start() {
    ...
    moveOrder.sort((a, b) => {
      ...
      if (aCommand.command !== bCommand.command) {
        ...
        applyMoveAttrs(
          IncrementMovePriorityAttr,
          this.scene.getField().find((p) => p.isActive() && p.getBattlerIndex() === a),
          null,
          aMove,
          aPriority
        );
        applyMoveAttrs(
          IncrementMovePriorityAttr,
          this.scene.getField().find((p) => p.isActive() && p.getBattlerIndex() === b),
          null,
          bMove,
          bPriority
        );
    ...
    for (const o of moveOrder) {
      ...
      case Command.FIGHT:
        ...
        const move =
            pokemon.getMoveset().find((m) => m.moveId === queuedMove.move) ||
    ...
    const handlePokemon = (pokemon: Pokemon) => {
      ...
      if (pokemon.summonData.disabledMove && !--pokemon.summonData.disabledTurns) {
        this.scene.pushPhase(
          new MessagePhase(
            this.scene,
            i18next.t("battle:notDisabled", {
              pokemonName: getPokemonNameWithAffix(pokemon),
              moveName: allMoves[pokemon.summonData.disabledMove].name
            })
          )
        );
      ...
      if (this.scene.arena.terrain.terrainType === TerrainType.GRASSY && pokemon.isGrounded()) {
        this.scene.unshiftPhase(
          new PokemonHealPhase(
            ...
            getPokemonMessage(pokemon, "'s HP was restored."),
            true
          )
        );
      }
...
export class CommonAnimPhase extends PokemonPhase {
  ...
  start() {
    new CommonBattleAnim(
      this.anim,
      this.getPokemon(),
      this.targetIndex !== undefined
        ? (this.player ? this.scene.getEnemyField() : this.scene.getPlayerField())[this.targetIndex]
        : this.getPokemon()
    ).play(this.scene, () => {
      this.end();
    });
...
export class MovePhase extends BattlePhase {
  ...
  canMove(): boolean {
    return (
      ...
      this.move.isUsable(this.pokemon, this.ignorePp) &&
      !!this.targets.length
    );
  }
  ...
  start() {
    ...
    if (moveTarget) {
      ...
      this.scene
        ...
        .forEach((p) => applyAbAttrs(RedirectMoveAbAttr, p, null, this.move.moveId, moveTarget));
    ...
    const doMove = () => {
      ...
      const targetedOpponents = this.pokemon
        .getOpponents()
        .filter((o) => this.targets.includes(o.getBattlerIndex()));
      ...
      if (this.cancelled || this.failed) {
        if (this.failed) {
          ...
          this.scene.eventTarget.dispatchEvent(
            new MoveUsedEvent(this.pokemon.id, this.move.getMove(), ppUsed)
          );
        }
      ...
      let success = this.move
        .getMove()
        .applyConditions(this.pokemon, targets[0], this.move.getMove());
      const cancelled = new Utils.BooleanHolder(false);
      let failedText = this.move
        .getMove()
        .getFailedText(this.pokemon, targets[0], this.move.getMove(), cancelled);
      if (success && this.scene.arena.isMoveWeatherCancelled(this.move.getMove())) {
        success = false;
      } else if (
        success &&
        this.scene.arena.isMoveTerrainCancelled(this.pokemon, this.targets, this.move.getMove())
      ) {
        success = false;
        if (failedText === null) {
      ...
      if (
        success ||
        [Moves.ROAR, Moves.WHIRLWIND, Moves.TRICK_OR_TREAT, Moves.FORESTS_CURSE].includes(
          this.move.moveId
        )
      ) {
    ...
    if (!this.followUp && this.pokemon.status && !this.pokemon.status.isPostTurn()) {
      ...
      if (activated) {
        this.scene.queueMessage(
          getPokemonMessage(
            this.pokemon,
            getStatusEffectActivationText(this.pokemon.status.effect)
          )
        );
        this.scene.unshiftPhase(
          new CommonAnimPhase(
  ...
  showMoveText(): void {
    if (this.move.getMove().hasAttr(ChargeAttr)) {
      const lastMove = this.pokemon.getLastXMoves();
      if (
        !lastMove.length ||
        lastMove[0].move !== this.move.getMove().id ||
        lastMove[0].result !== MoveResult.OTHER
      ) {
        this.scene.queueMessage(
...
export class MoveEffectPhase extends PokemonPhase {
  ...
  constructor(
    ...
    if (
      targets.includes(battlerIndex) &&
      this.move.getMove().moveTarget === MoveTarget.ALL_NEAR_OTHERS
    ) {
  ...
  start() {
    ...
    applyMoveAttrs(
      ...
      const targetHitChecks = Object.fromEntries(
        targets.map((p) => [p.getBattlerIndex(), this.hitCheck(p)])
      );
      ...
      new MoveAnim(move.id, user, this.getTarget().getBattlerIndex()).play(this.scene, () => {
        for (const target of targets) {
          ...
          const isProtected =
            ...
            target
              .findTags((t) => t instanceof ProtectedTag)
              .find((t) => target.lapseTag(t.tagType));
              ...
              applyFilteredMoveAttrs(
                ...
                if (hitResult !== HitResult.FAIL) {
                  const chargeEffect = !!move
                    .getAttrs(ChargeAttr)
                    .find((ca) => ca.usedChargeEffect(user, this.getTarget(), move));
                      ...
                      applyFilteredMoveAttrs(
                        ...
                        Utils.executeIf(!isProtected && !chargeEffect, () =>
                          applyFilteredMoveAttrs(
                            ...
                            return Utils.executeIf(
                              !target.isFainted() || target.canApplyAbility(),
                              () =>
                                applyPostDefendAbAttrs(
                                  PostDefendAbAttr,
                                  target,
        ...
        const postTarget = applyFilteredMoveAttrs(
          (attr: MoveAttr) =>
            attr instanceof MoveEffectAttr && attr.trigger === MoveEffectTrigger.POST_TARGET,
          user,
          null,
          move
        );
  ...
  hitCheck(target: Pokemon): boolean {
    ...
    if (user.turnData.hitsLeft < user.turnData.hitCount) {
      if (
        !this.move.getMove().hasFlag(MoveFlags.CHECK_ALL_HITS) ||
        user.hasAbilityWithAttr(MaxMultiHitAbAttr)
      ) {
        return true;
      }
    }
    ...
    if (
      ...
      (
        user
          ...
          .slice(1)
          .find(() => true)?.targets || []
      ).indexOf(target.getBattlerIndex()) !== -1
    ) {
      return true;
    }
    ...
    if (
      ...
      !this.move
        ...
        .some((hta) => hta.tagType === hiddenTag.tagType)
    ) {
      return false;
    }
    ...
    const targetEvasionLevel = new Utils.IntegerHolder(
      target.summonData.battleStats[BattleStat.EVA]
    );
    ...
    if (userAccuracyLevel.value !== targetEvasionLevel.value) {
      accuracyMultiplier.value =
        userAccuracyLevel.value > targetEvasionLevel.value
  ...
  getUserPokemon(): Pokemon {
    ...
    return (this.player ? this.scene.getPlayerField() : this.scene.getEnemyField())[
      this.fieldIndex
    ];
  }
...
export class MoveAnimTestPhase extends BattlePhase {
  ...
  constructor(scene: BattleScene, moveQueue?: Moves[]) {
    ...
    this.moveQueue = moveQueue || Object.values(Moves).slice(1);
  }
...
export class StatChangePhase extends PokemonPhase {
  ...
  start() {
    ...
    const filteredStats = this.stats
      .map((s) => (s !== BattleStat.RAND ? s : this.getRandomStat()))
      .filter((stat) => {
        const cancelled = new Utils.BooleanHolder(false);
    ...
    const relLevels = filteredStats.map(
      (stat) =>
        (levels.value >= 1
          ? Math.min(battleStats[stat] + levels.value, 6)
          : Math.max(battleStats[stat] + levels.value, -6)) - battleStats[stat]
    );
    ...
    const end = () => {
      ...
      for (const stat of filteredStats) {
        pokemon.summonData.battleStats[stat] = Math.max(
          Math.min(pokemon.summonData.battleStats[stat] + levels.value, 6),
          -6
        );
      }
    ...
    if (relLevels.filter((l) => l).length && this.scene.moveAnimations) {
      ...
      const tileY =
        ((this.player ? 148 : 84) + (levels.value >= 1 ? 160 : 0)) *
        pokemon.getSpriteScale() *
        this.scene.field.scale;
      ...
      const statSprite = this.scene.add.tileSprite(
        tileX,
        tileY,
        tileWidth,
        tileHeight,
  ...
  getRandomStat(): BattleStat {
    const allStats = Object.values(BattleStat);
  ...
  getStatChangeMessages(stats: BattleStat[], levels: integer, relLevels: integer[]): string[] {
    ...
    Object.keys(relLevelStatIndexes).forEach((rl) => {
      ...
      if (relLevelStats.length > 1) {
        statsFragment =
          relLevelStats.length >= 5
            ? "stats"
            : `${relLevelStats
              .slice(0, -1)
              .map((s) => getBattleStatName(s))
              .join(
                ", "
              )}${relLevelStats.length > 2 ? "," : ""} and ${getBattleStatName(relLevelStats[relLevelStats.length - 1])}`;
      } else {
      ...
      messages.push(
        getPokemonMessage(
          this.getPokemon(),
          `'s ${statsFragment} ${getBattleStatLevelChangeDescription(Math.abs(parseInt(rl)), levels >= 1)}!`
        )
      );
    });
  ...
  start() {
    ...
    if (!pokemon.status) {
      if (pokemon.trySetStatus(this.statusEffect, false, this.sourcePokemon)) {
        ...
        new CommonBattleAnim(CommonAnim.POISON + (this.statusEffect - 1), pokemon).play(
          this.scene,
          () => {
            this.scene.queueMessage(
              getPokemonMessage(
                pokemon,
                getStatusEffectObtainText(this.statusEffect, this.sourceText)
              )
            );
      ...
      this.scene.queueMessage(
        getPokemonMessage(pokemon, getStatusEffectOverlapText(this.statusEffect))
      );
    }
    this.end();
  }
    ...
    if (pokemon?.isActive(true) && pokemon.status && pokemon.status.isPostTurn()) {
      ...
      if (!cancelled.value) {
        ...
        if (damage) {
          ...
          this.scene.damageNumberHandler.add(
            this.getPokemon(),
            pokemon.damage(damage, false, true)
          );
          pokemon.updateInfo();
        }
        new CommonBattleAnim(CommonAnim.POISON + (pokemon.status.effect - 1), pokemon).play(
          this.scene,
          () => this.end()
        );
      } else {
        ...
        this.scene.ui.showDialogue(
          ...
          () => {
            this.scene.addEnemyModifier(
                getModifierType(modifierTypes.MINI_BLACK_HOLE).newModifier(
                  pokemon
...
export class FaintPhase extends PokemonPhase {
  ...
  doFaint(): void {
    ...
    if (this.player) {
      const nonFaintedLegalPartyMembers = this.scene
        .getParty()
        .filter((p) => p.isAllowedInBattle());
      ...
      if (!nonFaintedPartyMemberCount) {
        ...
        nonFaintedPartyMemberCount >= this.scene.currentBattle.getBattlerCount() ||
        (this.scene.currentBattle.double && !nonFaintedLegalPartyMembers[0].isActive(true))
      ) {
        ...
        const hasReservePartyMember = !!this.scene
          ...
          .filter(
            (p) =>
              p.isActive() &&
              !p.isOnField() &&
              p.trainerSlot === (pokemon as EnemyPokemon).trainerSlot
          ).length;
        if (hasReservePartyMember) {
          this.scene.pushPhase(
            new SwitchSummonPhase(this.scene, this.fieldIndex, -1, false, false, false)
          );
        }
      }
    }
    ...
    if (this.scene.currentBattle.double) {
      ...
      if (allyPokemon.isActive(true)) {
        ...
        do {
          targetingMovePhase = this.scene.findPhase(
            (mp) =>
              mp instanceof MovePhase &&
              mp.targets.length === 1 &&
              mp.targets[0] === pokemon.getBattlerIndex() &&
    ...
    case BattleSpec.FINAL_BOSS:
      if (!this.player) {
        ...
        if (enemy.formIndex) {
          ...
          enemy.hp++;
          this.scene.unshiftPhase(
            new DamagePhase(this.scene, enemy.getBattlerIndex(), 0, HitResult.OTHER)
          );
          this.end();
        }
        return true;
...
export class VictoryPhase extends PokemonPhase {
  ...
  start() {
    ...
    const party = this.scene.getParty();
    const expShareModifier = this.scene.findModifier(
      (m) => m instanceof ExpShareModifier
    ) as ExpShareModifier;
    const expBalanceModifier = this.scene.findModifier(
      (m) => m instanceof ExpBalanceModifier
    ) as ExpBalanceModifier;
    const multipleParticipantExpBonusModifier = this.scene.findModifier(
    ...
    const expPartyMembers = nonFaintedPartyMembers.filter(
      (p) => p.level < this.scene.getMaxExpLevel()
    );
        ...
        if (exp) {
          ...
          this.scene.unshiftPhase(
            expPartyMembers[pm].isOnField()
              ? new ExpPhase(this.scene, partyMemberIndex, exp)
              : new ShowPartyExpBarPhase(this.scene, partyMemberIndex, exp)
          );
        }
      }
    }
    ...
    if (
      !this.scene
        .getEnemyParty()
        .find((p) => (this.scene.currentBattle.battleType ? !p.isFainted(true) : p.isOnField()))
    ) {
      ...
      if (
        ...
        if (this.scene.currentBattle.waveIndex % 10) {
          ...
          if (
            ...
            this.scene.pushPhase(
              new ModifierRewardPhase(this.scene, modifierTypes.GOLDEN_POKEBALL)
            );
          }
        } else {
          const superExpWave = !this.scene.gameMode.isEndless
          ...
          if (
            ...
            (this.scene.currentBattle.waveIndex <= 500 ||
              this.scene.currentBattle.waveIndex % 30 === superExpWave)
          ) {
            this.scene.pushPhase(
          ...
          if (
            this.scene.currentBattle.waveIndex <= 150 &&
            !(this.scene.currentBattle.waveIndex % 50)
          ) {
            this.scene.pushPhase(
              new ModifierRewardPhase(this.scene, modifierTypes.GOLDEN_POKEBALL)
            );
      ...
      () => {
        ...
        const showMessage = () => {
          ...
          showMessageOrEnd = () =>
            this.scene.ui.showDialogue(
              message,
              this.scene.currentBattle.trainer.getName(),
              null,
              originalFunc
            );
...
export class MoneyRewardPhase extends BattlePhase {
  ...
  start() {
    const moneyAmount = new Utils.IntegerHolder(
      this.scene.getWaveMoneyAmount(this.moneyMultiplier)
    );
  ...
  doReward(): Promise<void> {
    return new Promise<void>((resolve) => {
      ...
      this.scene.addModifier(newModifier).then(() => {
        ...
        this.scene.ui.setMode(Mode.MESSAGE);
        this.scene.ui.showText(
          `${this.species.name} beat ${this.scene.gameMode.getName()} Mode for the first time!\nYou received ${newModifier.type.name}!`,
          null,
          () => {
            resolve();
          },
...
export class GameOverPhase extends BattlePhase {
  ...
  start() {
    ...
    if (this.victory && this.scene.gameMode.isEndless) {
      this.scene.ui.showDialogue(
        i18next.t("PGMmiscDialogue:ending_endless"),
        i18next.t("PGMmiscDialogue:ending_name"),
        0,
        () => this.handleGameOver()
      );
      ...
      this.scene.time.delayedCall(1000, () => {
        ...
        this.scene.ui.fadeOut(fadeDuration).then(() => {
          ...
          const clear = (endCardPhase?: EndCardPhase) => {
            ...
            if (this.victory && newClear) {
              for (const species of this.firstRibbons) {
                this.scene.unshiftPhase(
                  new RibbonModifierRewardPhase(this.scene, modifierTypes.VOUCHER_PLUS, species)
                );
              }
              if (!firstClear) {
                this.scene.unshiftPhase(
                  new GameOverModifierRewardPhase(this.scene, modifierTypes.VOUCHER_PREMIUM)
                );
              }
            }
          ...
          if (this.victory && this.scene.gameMode.isClassic) {
            const message =
              miscDialogue.ending[this.scene.gameData.gender === PlayerGender.FEMALE ? 0 : 1];
              ...
              this.scene.ui.fadeIn(500).then(() => {
                this.scene.charSprite
                  .showCharacter(
                    ...
                    getCharVariantFromDialogue(
                      miscDialogue.ending[
                        this.scene.gameData.gender === PlayerGender.FEMALE ? 0 : 1
                      ]
                    )
                  )
                  .then(() => {
...
export class EndCardPhase extends Phase {
  ...
  start(): void {
    ...
    this.endCard = this.scene.add.image(
      0,
      0,
      `end_${this.scene.gameData.gender === PlayerGender.FEMALE ? "f" : "m"}`
    );
...
export class UnlockPhase extends Phase {
  ...
  start(): void {
    this.scene.time.delayedCall(2000, () => {
      ...
      this.scene.ui.setMode(Mode.MESSAGE);
      this.scene.ui.showText(
        `${getUnlockableName(this.unlockable)}\nhas been unlocked.`,
        null,
        () => {
          this.scene.time.delayedCall(1500, () => this.scene.arenaBg.setVisible(true));
          this.end();
...
export class SwitchPhase extends BattlePhase {
  ...
  start() {
    ...
    if (
      this.isModal &&
      !this.scene.getParty().filter((p) => p.isAllowedInBattle() && !p.isActive(true)).length
    ) {
      return super.end();
    }
...
export class ExpPhase extends PlayerPartyMemberPokemonPhase {
  ...
  start() {
    ...
    this.scene.ui.showText(
      i18next.t("battle:expGain", { pokemonName: pokemon.name, exp: exp.value }),
      null,
      () => {
        ...
        if (newLevel > lastLevel) {
          this.scene.unshiftPhase(
            new LevelUpPhase(this.scene, this.partyMemberIndex, lastLevel, newLevel)
          );
        }
      ...
      if (newLevel > lastLevel) {
        ...
        this.scene.partyExpBar
          .showPokemonExp(
            pokemon,
            exp.value,
            this.scene.expParty === ExpNotification.ONLY_LEVEL_UP,
            newLevel
          )
          .then(() => {
...
export class LevelUpPhase extends PlayerPartyMemberPokemonPhase {
  ...
  start() {
    ...
    if (this.scene.expParty === ExpNotification.DEFAULT) {
      this.scene.playSound("level_up_fanfare");
      this.scene.ui.showText(
        i18next.t("battle:levelUp", { pokemonName: this.getPokemon().name, level: this.level }),
        null,
        () =>
          this.scene.ui
            .getMessageHandler()
    ...
    if (!pokemon.pauseEvolutions) {
      ...
      if (evolution) {
        this.scene.unshiftPhase(
          new EvolutionPhase(this.scene, pokemon as PlayerPokemon, evolution, this.lastLevel)
        );
      }
    }
  }
...
export class LearnMovePhase extends PlayerPartyMemberPokemonPhase {
  ...
  start() {
    ...
    const emptyMoveIndex =
      pokemon.getMoveset().length < 4
        ? pokemon.getMoveset().length
        : pokemon.getMoveset().findIndex((m) => m === null);

    const messageMode =
      this.scene.ui.getHandler() instanceof EvolutionSceneHandler
    ...
    if (emptyMoveIndex > -1) {
      ...
      initMoveAnim(this.scene, this.moveId).then(() => {
        loadMoveAnimAssets(this.scene, [this.moveId], true).then(() => {
          this.scene.ui.setMode(messageMode).then(() => {
            this.scene.playSound("level_up_fanfare");
            this.scene.ui.showText(
              i18next.t("battle:learnMove", { pokemonName: pokemon.name, moveName: move.name }),
              null,
              () => {
              ...
              messageMode === Mode.EVOLUTION_SCENE ? 1000 : null,
              true
            );
          });
        });
      ...
      this.scene.ui.setMode(messageMode).then(() => {
        this.scene.ui.showText(
          i18next.t("battle:learnMovePrompt", { pokemonName: pokemon.name, moveName: move.name }),
          null,
          () => {
            this.scene.ui.showText(
              i18next.t("battle:learnMoveLimitReached", { pokemonName: pokemon.name }),
              null,
              () => {
                this.scene.ui.showText(
                  i18next.t("battle:learnMoveReplaceQuestion", { moveName: move.name }),
                  null,
                  () => {
                    const noHandler = () => {
                      this.scene.ui.setMode(messageMode).then(() => {
                    ...
                    this.scene.ui.setModeWithoutClear(
                      ...
                      () => {
                        this.scene.ui.setMode(messageMode);
                        this.scene.ui.showText(
                          i18next.t("battle:learnMoveForgetQuestion"),
                          null,
                          () => {
                            this.scene.ui.setModeWithoutClear(
                              Mode.SUMMARY,
                              ...
                              (moveIndex: integer) => {
                                ...
                                this.scene.ui.setMode(messageMode).then(() => {
                                  this.scene.ui.showText(
                                    ...
                                    () => {
                                      this.scene.ui.showText(
                                        i18next.t("battle:learnMoveForgetSuccess", {
                                          pokemonName: pokemon.name,
...
export class PokemonHealPhase extends CommonAnimPhase {
  ...
  start() {
    if (
      !this.skipAnim &&
      (this.revive || this.getPokemon().hp) &&
      this.getPokemon().getHpRatio() < 1
    ) {
      super.start();
    } else {
      this.end();
    ...
    if (healOrDamage) {
      ...
      const healAmount = new Utils.NumberHolder(
        Math.floor(this.hpHealed * hpRestoreMultiplier.value)
      );
    ...
    if (this.healStatus && lastStatusEffect && !hasMessage) {
      this.scene.queueMessage(
        getPokemonMessage(pokemon, getStatusEffectHealText(lastStatusEffect))
      );
    }
...
export class AttemptCapturePhase extends PokemonPhase {
  ...
  start() {
    ...
    const statusMultiplier = pokemon.status
      ? getStatusEffectCatchRateMultiplier(pokemon.status.effect)
      : 1;
        ...
        this.scene.tweens.add({
          ...
          onComplete: () => {
              ...
              const shakeCounter = this.scene.tweens.addCounter({
                ...
                onRepeat: () => {
                  if (!pokemon.species.isObtainable()) {
                    ...
                    const pbTint = this.scene.add.sprite(
                      this.pokeball.x,
                      this.pokeball.y,
                      "pb",
                      "pb"
            ...
            this.scene.time.delayedCall(250, () =>
              doPokeballBounceAnim(this.scene, this.pokeball, 16, 72, 350, doShake)
            );
          }
        });
      }
  ...
  failCatch(shakeCount: integer) {
    ...
    const speciesForm = !pokemon.fusionSpecies
    ...
    this.scene.ui.showText(
      i18next.t("battle:pokemonCaught", { pokemonName: pokemon.name }),
      null,
      () => {
        const end = () => {
        ...
        Promise.all([pokemon.hideInfo(), this.scene.gameData.setPokemonCaught(pokemon)]).then(
          () => {
            if (this.scene.getParty().length === 6) {
              const promptRelease = () => {
                this.scene.ui.showText(
                  i18next.t("battle:partyFull", { pokemonName: pokemon.name }),
                  null,
                  () => {
...
export class AttemptRunPhase extends PokemonPhase {
  ...
  start() {
    ...
    const enemySpeed =
      enemyField.reduce(
        (total: integer, enemyPokemon: Pokemon) => total + enemyPokemon.getStat(Stat.SPD),
        0
      ) / enemyField.length;

    const escapeChance = new Utils.IntegerHolder(
      ((playerPokemon.getStat(Stat.SPD) * 128) / enemySpeed +
        30 * this.scene.currentBattle.escapeAttempts++) %
        256
    );
...
export class SelectModifierPhase extends BattlePhase {
  ...
  start() {
    ...
    const modifierSelectCallback = (rowCursor: integer, cursor: integer) => {
      ...
      default:
        ...
        const shopOption =
            shopOptions[
              rowCursor > 2 || shopOptions.length <= SHOP_OPTIONS_ROW_LIMIT
                ? cursor
                : cursor + SHOP_OPTIONS_ROW_LIMIT
            ];
  ...
  getRerollCost(typeOptions: ModifierTypeOption[], lockRarities: boolean): integer {
    ...
    return Math.min(
      Math.ceil(this.scene.currentBattle.waveIndex / 10) *
        baseValue *
        Math.pow(2, this.rerollCount),
...
export class ScanIvsPhase extends PokemonPhase {
  ...
  start() {
    ...
    this.scene.ui.showText(
      i18next.t("battle:ivScannerUseQuestion", { pokemonName: pokemon.name }),
      null,
      () => {
        this.scene.ui.setMode(
          Mode.CONFIRM,
...
export class TrainerMessageTestPhase extends BattlePhase {
  ...
  start() {
    ...
    for (const t of Object.keys(trainerConfigs)) {
      ...
      if (
        this.trainerTypes.length &&
        !this.trainerTypes.find((tt) => tt === (type as TrainerType))
      ) {
        continue;
      }
