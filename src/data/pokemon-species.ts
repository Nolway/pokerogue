...
export function getFusedSpeciesName(speciesAName: string, speciesBName: string): string {
  ...
  const [speciesAPrefixMatch, speciesBPrefixMatch] = [speciesAName, speciesBName].map((n) =>
    /^(?:[^ ]+) /.exec(n)
  );
  const [speciesAPrefix, speciesBPrefix] = [speciesAPrefixMatch, speciesBPrefixMatch].map((m) =>
    m ? m[0] : ""
  );
  ...
  const [speciesASuffixMatch, speciesBSuffixMatch] = [speciesAName, speciesBName].map((n) =>
    / (?:[^ ]+)$/.exec(n)
  );
  const [speciesASuffix, speciesBSuffix] = [speciesASuffixMatch, speciesBSuffixMatch].map((m) =>
    m ? m[0] : ""
  );
...
export abstract class PokemonSpeciesForm {
  ...
  getRootSpeciesId(forStarter: boolean = false): Species {
    ...
    while (
      ...
      (!forStarter || !speciesStarters.hasOwnProperty(ret))
    ) {
  ...
  getSpriteId(
    ...
    const showGenderDiffs =
      this.genderDiffs &&
      female &&
      ![SpeciesFormKey.MEGA, SpeciesFormKey.GIGANTAMAX].find((k) => formSpriteKey === k);
  ...
  getIconAtlasKey(formIndex?: integer, shiny?: boolean, variant?: integer): string {
    ...
    const isVariant =
      shiny && variantData[variantDataIndex] && variantData[variantDataIndex][variant];
  ...
  validateStarterMoveset(moveset: StarterMoveset, eggMoves: integer): boolean {
    ...
    for (const moveId of moveset) {
      ...
      if (
        ...
        !pokemonSpeciesLevelMoves[this.speciesId].find((lm) => lm[0] <= 5 && lm[1] === moveId)
      ) {
        return false;
      }
    }
        ...
        if (variantSet && variantSet[variant] === 1) {
          const populateVariantColors = (key: string): Promise<void> => {
            return new Promise((resolve) => {
              ...
              scene
                .cachedFetch(`./images/pokemon/variant/${spritePath}.json`)
                .then((res) => res.json())
                .then((c) => {
  ...
  cry(
    ...
    let cry = scene.sound.get(cryKey);
...
export default class PokemonSpecies extends PokemonSpeciesForm implements Localizable {
  ...
  getSpeciesForLevel(
    ...
    const evolutionPool = new Map<number, Species>();
    let totalWeight = 0;
        ...
        if (ev.wildDelay === SpeciesWildEvolutionDelay.NONE) {
          if (strength === PartyMemberStrength.STRONGER) {
            ...
            evolutionChance = Math.min(
              minChance +
                easeInFunc(Math.min(level - ev.level, maxLevelDiff) / maxLevelDiff) *
                  (1 - minChance),
              1
            );
          }
          ...
          evolutionChance = Math.min(
            0.65 *
              easeInFunc(
                Math.min(Math.max(level - evolutionLevel, 0), preferredMinLevel) /
                  preferredMinLevel
              ) +
              0.35 *
                easeOutFunc(
                  Math.min(Math.max(level - evolutionLevel, 0), preferredMinLevel * 2.5) /
        ...
        ret.push([
          ...
          Math.min(
            Math.max(
              evolution.level +
                Math.round(
                  Utils.randSeedGauss(0.5, 1 + levelDiff * 0.2) *
