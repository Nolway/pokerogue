...
type BiomeLinks = Record<integer, Biome | (Biome | [Biome, integer])[]>;

type BiomeDepths = Record<integer, [integer, integer]>;
...
export type SpeciesTree = Record<integer, Species[]>;

export type PokemonPools = Record<integer, (Species | SpeciesTree)[]>;

export type BiomeTierPokemonPools = Record<integer, PokemonPools>;

export type BiomePokemonPools = Record<integer, BiomeTierPokemonPools>;

export type BiomeTierTrainerPools = Record<integer, TrainerType[]>;

export type BiomeTrainerPools = Record<integer, BiomeTierTrainerPools>;
...
export const biomePokemonPools: BiomePokemonPools = {
  ...
  [Biome.BEACH]: {
    ...
    [BiomePoolTier.RARE]: {
      ...
      [TimeOfDay.NIGHT]: [],
      [TimeOfDay.ALL]: [
        { 1: [Species.QUAXLY], 16: [Species.QUAXWELL], 36: [Species.QUAQUAVAL] },
        Species.TATSUGIRI
      ]
    },
    [BiomePoolTier.SUPER_RARE]: {
...
export function initBiomes() {
  ...
  const traverseBiome = (biome: Biome, depth: integer) => {
    ...
    for (const linkedBiomeEntry of linkedBiomes) {
      const linkedBiome = !Array.isArray(linkedBiomeEntry) ? linkedBiomeEntry : linkedBiomeEntry[0];
  ...
  for (const biome of Object.values(Biome)) {
    ...
    for (const tier of Object.values(BiomePoolTier)) {
  ...
  for (const pb of pokemonBiomes) {
    ...
    if (
      ...
      !speciesEvolutions.filter(
        (es) =>
          !!(pokemonBiomes.find((p) => p[0] === es.speciesId)[3] as any[]).filter(
            (b) => b[0] !== Biome.END
          ).length
      ).length
    ) {
        ...
        for (let t = 0; t < biomeTierPool.length; t++) {
          ...
          for (let es = 0; es < existingSpeciesIds.length; es++) {
            ...
            if (
              ...
              speciesEvolutions &&
              speciesEvolutions.find((se) => se.speciesId === existingSpeciesId)
            ) {
              treeIndex = t;
              arrayIndex = es;
              break;
  ...
  /*for (let pokemon of allSpecies) {
    ...
    for (let b of Object.values(Biome)) {
