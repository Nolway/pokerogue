...
export class Ability {
  ...
  attr<T extends new (...args: any[]) => AbAttr>(
    AttrType: T,
  ...
  conditionalAttr<T extends new (...args: any[]) => AbAttr>(
...
type AbAttrApplyFunc<TAttr extends AbAttr> = (
  attr: TAttr,
  passive: boolean,
...
type PokemonStatChangeCondition = (
  target: Pokemon,
        ...
        if (!simulated) {
          ...
          pokemon.scene.unshiftPhase(
            new PokemonHealPhase(
              pokemon.scene,
              pokemon.getBattlerIndex(),
              Math.max(Math.floor(pokemon.getMaxHp() / 4), 1),
...
export class PostDefendDisguiseAbAttr extends PostDefendAbAttr {
  applyPostDefend(
    ...
    if (
      pokemon.formIndex === 0 &&
      pokemon.battleData.hitCount !== 0 &&
      (move.category === MoveCategory.SPECIAL || move.category === MoveCategory.PHYSICAL)
    ) {
...
export class PostDefendHpGatedStatChangeAbAttr extends PostDefendAbAttr {
  ...
  applyPostDefend(
    ...
    const lastAttackReceived =
      pokemon.turnData.attacksReceived[pokemon.turnData.attacksReceived.length - 1];
    ...
    if (
      ...
      const effect =
        this.effects.length === 1
          ? this.effects[0]
          : this.effects[pokemon.randSeedInt(this.effects.length)];
...
export class PostDefendCritStatChangeAbAttr extends PostDefendAbAttr {
  ...
  getCondition(): AbAttrCondition {
    return (pokemon: Pokemon) =>
      pokemon.turnData.attacksReceived.length &&
...
export class PostDefendPerishSongAbAttr extends PostDefendAbAttr {
  ...
  applyPostDefend(
    ...
    if (move.checkFlag(MoveFlags.MAKES_CONTACT, attacker, pokemon)) {
      if (
        pokemon.getTag(BattlerTagType.PERISH_SONG) ||
...
export class FieldMultiplyBattleStatAbAttr extends AbAttr {
  ...
  applyFieldBattleStat(
    ...
    if (
      this.stat === stat &&
      checkedPokemon
        .getAbilityAttrs(FieldMultiplyBattleStatAbAttr)
        .every((attr) => (attr as FieldMultiplyBattleStatAbAttr).stat !== stat)
    ) {
  ...
  applyPostAttack(
    ...
    return new Promise<boolean>((resolve) => {
      if (
        ...
        (!this.condition || this.condition(pokemon, defender, move))
      ) {
        const heldItems = this.getTargetHeldItems(defender).filter((i) =>
          i.getTransferrable(false)
        );
        if (heldItems.length) {
  ...
  applyPostDefend(
    ...
    return new Promise<boolean>((resolve) => {
      if (
        ...
        const heldItems = this.getTargetHeldItems(attacker).filter((i) =>
          i.getTransferrable(false)
        );
        if (heldItems.length) {
          const stolenItem = heldItems[pokemon.randSeedInt(heldItems.length)];
...
class PostVictoryStatChangeAbAttr extends PostVictoryAbAttr {
  ...
  applyPostVictory(pokemon: Pokemon, passive: boolean, args: any[]): boolean | Promise<boolean> {
    ...
    pokemon.scene.unshiftPhase(
      new StatChangePhase(pokemon.scene, pokemon.getBattlerIndex(), true, [stat], this.levels)
    );
...
export class CopyFaintedAllyAbilityAbAttr extends PostKnockOutAbAttr {
  ...
  applyPostKnockOut(
    ...
    if (
      ...
      !knockedOut.getAbility().hasAttr(UncopiableAbilityAbAttr)
    ) {
      pokemon.summonData.ability = knockedOut.getAbility().id;
      pokemon.scene.queueMessage(
        getPokemonMessage(
          knockedOut,
          `'s ${allAbilities[knockedOut.getAbility().id].name} was taken over!`
        )
      );
      return true;
    }
...
export class PostIntimidateStatChangeAbAttr extends AbAttr {
  ...
  apply(pokemon: Pokemon, passive: boolean, cancelled: Utils.BooleanHolder, args: any[]): boolean {
    pokemon.scene.pushPhase(
      new StatChangePhase(pokemon.scene, pokemon.getBattlerIndex(), false, this.stats, this.levels)
    );
...
export class PostSummonStatChangeAbAttr extends PostSummonAbAttr {
  ...
  applyPostSummon(pokemon: Pokemon, passive: boolean, args: any[]): boolean {
    ...
    for (const opponent of pokemon.getOpponents()) {
      ...
      if (!cancelled.value) {
        const statChangePhase = new StatChangePhase(
          pokemon.scene,
          opponent.getBattlerIndex(),
          false,
          this.stats,
    ...
    if (target.isActive(true)) {
      target.scene.unshiftPhase(
        new PokemonHealPhase(
          ...
          getPokemonMessage(target, ` drank down all the\nmatcha that ${pokemon.name} made!`),
          true,
          !this.showAnim
        )
      );
...
export class DownloadAbAttr extends PostSummonAbAttr {
  ...
  applyPostSummon(pokemon: Pokemon, passive: boolean, args: any[]): boolean {
    ...
    if (this.enemyDef > 0 && this.enemySpDef > 0) {
      ...
      pokemon.scene.unshiftPhase(
        new StatChangePhase(pokemon.scene, pokemon.getBattlerIndex(), false, this.stats, 1)
      );
      return true;
    }
...
export class PostSummonTransformAbAttr extends PostSummonAbAttr {
  ...
  applyPostSummon(pokemon: Pokemon, passive: boolean, args: any[]): boolean {
    ...
    pokemon.summonData.moveset = target
      .getMoveset()
...
export class ConfusionOnStatusEffectAbAttr extends PostAttackAbAttr {
  ...
  applyPostAttack(
    ...
    if (this.effects.indexOf(args[0]) > -1 && !defender.isFainted()) {
      return defender.addTag(
        BattlerTagType.CONFUSED,
        pokemon.randSeedInt(3, 2),
        move.id,
        defender.id
      );
    }
    ...
    for (const opponent of pokemon.getOpponents()) {
      for (const move of opponent.moveset) {
        ...
        if (
          ...
          pokemon.getAttackTypeEffectiveness(move.getMove().type, opponent, true) >= 2
        ) {
          return true;
        }
        // move is a OHKO
...
export class ForewarnAbAttr extends PostSummonAbAttr {
  ...
  applyPostSummon(pokemon: Pokemon, passive: boolean, args: any[]): boolean {
    ...
    pokemon.scene.queueMessage(
      getPokemonMessage(pokemon, " was forewarned about " + maxMove + "!")
    );
    return true;
  }
}
  ...
  applyPostSummon(pokemon: Pokemon, passive: boolean, args: any[]): boolean {
    for (const opponent of pokemon.getOpponents()) {
      pokemon.scene.queueMessage(
        getPokemonMessage(
          pokemon,
          " frisked " + opponent.name + "'s " + opponent.getAbility().name + "!"
        )
      );
    ...
    if (pokemon.getHpRatio() < 1) {
      ...
      scene.unshiftPhase(
        new PokemonHealPhase(
          scene,
          pokemon.getBattlerIndex(),
          Math.max(Math.floor(pokemon.getMaxHp() / (16 / this.healFactor)), 1),
          getPokemonMessage(pokemon, `'s ${abilityName}\nrestored its HP a little!`),
          true
        )
      );
  ...
  applyPostWeatherLapse(
    ...
    if (pokemon.getHpRatio() < 1) {
      ...
      pokemon.damageAndUpdate(
        Math.ceil(pokemon.getMaxHp() / (16 / this.damageFactor)),
        HitResult.OTHER
      );
      return true;
    }
  ...
  applyPostTurn(pokemon: Pokemon, passive: boolean, args: any[]): boolean {
    ...
    if (this.target.status) {
      this.target.scene.queueMessage(
        getPokemonMessage(this.target, getStatusEffectHealText(this.target.status.effect))
      );
...
export class MoodyAbAttr extends PostTurnAbAttr {
  ...
  applyPostTurn(pokemon: Pokemon, passive: boolean, args: any[]): boolean {
    ...
    if (increaseStatArray.length > 0) {
      ...
      pokemon.scene.unshiftPhase(
        new StatChangePhase(pokemon.scene, pokemon.getBattlerIndex(), true, [increaseStat], 2)
      );
    }
    if (decreaseStatArray.length > 0) {
      const decreaseStat = selectableStats[Utils.randInt(selectableStats.length)];
      pokemon.scene.unshiftPhase(
        new StatChangePhase(pokemon.scene, pokemon.getBattlerIndex(), true, [decreaseStat], -1)
      );
    }
    return true;
  }
...
export class PostTurnStatChangeAbAttr extends PostTurnAbAttr {
  ...
  applyPostTurn(pokemon: Pokemon, passive: boolean, args: any[]): boolean {
    pokemon.scene.unshiftPhase(
      new StatChangePhase(pokemon.scene, pokemon.getBattlerIndex(), true, this.stats, this.levels)
    );
    return true;
  }
}
  ...
  applyPostMoveUsed(
    ...
    if (
      ...
      if (move.getMove() instanceof AttackMove || move.getMove() instanceof StatusMove) {
        ...
        dancer.scene.unshiftPhase(
          new MovePhase(dancer.scene, dancer, [dancer.getBattlerIndex()], move, true)
        );
      }
      return true;
    }
  ...
  applyCheckTrapped(
    ...
    if (this.arenaTrapCondition(pokemon, otherPokemon)) {
      if (
        ...
        (otherPokemon.getTypes(true).includes(Type.STELLAR) &&
          otherPokemon.getTypes().includes(Type.GHOST))
      ) {
        trapped.value = false;
        return false;
...
export class PostFaintClearWeatherAbAttr extends PostFaintAbAttr {
  ...
  applyPostFaint(
    ...
    case WeatherType.HARSH_SUN:
      if (
        pokemon.hasAbility(Abilities.DESOLATE_LAND) &&
          pokemon.scene.getField(true).filter((p) => p.hasAbility(Abilities.DESOLATE_LAND))
            .length === 0
      ) {
        turnOffWeather = true;
      }
    ...
    case WeatherType.HEAVY_RAIN:
      if (
        pokemon.hasAbility(Abilities.PRIMORDIAL_SEA) &&
          pokemon.scene.getField(true).filter((p) => p.hasAbility(Abilities.PRIMORDIAL_SEA))
            .length === 0
      ) {
        turnOffWeather = true;
      }
    ...
    case WeatherType.STRONG_WINDS:
      if (
        pokemon.hasAbility(Abilities.DELTA_STREAM) &&
          pokemon.scene.getField(true).filter((p) => p.hasAbility(Abilities.DELTA_STREAM))
            .length === 0
      ) {
        turnOffWeather = true;
      }
...
export class ReduceStatusEffectDurationAbAttr extends AbAttr {
  ...
  apply(pokemon: Pokemon, passive: boolean, cancelled: Utils.BooleanHolder, args: any[]): boolean {
    if (args[0] === this.statusEffect) {
      (args[1] as Utils.IntegerHolder).value = Math.floor(
        (args[1] as Utils.IntegerHolder).value / 2
      );
      return true;
    }
...
export class SuppressFieldAbilitiesAbAttr extends AbAttr {
  ...
  apply(pokemon: Pokemon, passive: boolean, cancelled: Utils.BooleanHolder, args: any[]): boolean {
    ...
    if (
      ...
      !ability.hasAttr(SuppressFieldAbilitiesAbAttr)
    ) {
      cancelled.value = true;
      return true;
    }
...
export class IgnoreTypeImmunityAbAttr extends AbAttr {
  ...
  apply(pokemon: Pokemon, passive: boolean, cancelled: Utils.BooleanHolder, args: any[]): boolean {
    if (
      this.defenderType === (args[1] as Type) &&
      this.allowedMoveTypes.includes(args[0] as Type)
    ) {
      cancelled.value = true;
      return true;
    }
...
export class IgnoreTypeStatusEffectImmunityAbAttr extends AbAttr {
  ...
  apply(pokemon: Pokemon, passive: boolean, cancelled: Utils.BooleanHolder, args: any[]): boolean {
    if (
      ...
      this.defenderType.includes(args[1] as Type)
    ) {
      cancelled.value = true;
      return true;
    }
  ...
  return new Promise((resolve) => {
    ...
    const applyAbAttr = (attr: TAttr) => {
      ...
      const onApplySuccess = () => {
        ...
        if (!quiet) {
          const message = attr.getTriggerMessage(
            pokemon,
            (!passive ? pokemon.getAbility() : pokemon.getPassiveAbility()).name,
            args
          );
          if (message) {
            if (isAsync) {
              pokemon.scene.ui.showText(
                message,
                null,
                () => pokemon.scene.ui.showText(null, 0),
                null,
                true
              );
            } else {
...
export function applyAbAttrs(
  ...
  return applyAbAttrsInternal<AbAttr>(
    attrType,
    pokemon,
    (attr, passive) => attr.apply(pokemon, passive, cancelled, args),
    args
  );
}
...
export function applyPostBattleInitAbAttrs(
  ...
  return applyAbAttrsInternal<PostBattleInitAbAttr>(
    attrType,
    pokemon,
    (attr, passive) => attr.applyPostBattleInit(pokemon, passive, args),
    args
  );
}

export function applyPreDefendAbAttrs(
  attrType: { new(...args: any[]): PreDefendAbAttr },
  pokemon: Pokemon,
  attacker: Pokemon,
  move: Move,
  cancelled: Utils.BooleanHolder,
  ...
  return applyAbAttrsInternal<PreDefendAbAttr>(
    attrType,
    pokemon,
    (attr, passive) => attr.applyPreDefend(pokemon, passive, attacker, move, cancelled, args),
    args,
    false,
    false,
    simulated
...
export function applyPostDefendAbAttrs(
  ...
  return applyAbAttrsInternal<PostDefendAbAttr>(
    attrType,
    pokemon,
    (attr, passive) => attr.applyPostDefend(pokemon, passive, attacker, move, hitResult, args),
    args
  );
}

export function applyPostMoveUsedAbAttrs(
  attrType: { new(...args: any[]): PostMoveUsedAbAttr },
  pokemon: Pokemon,
  move: PokemonMove,
  source: Pokemon,
  targets: BattlerIndex[],
  ...
  return applyAbAttrsInternal<PostMoveUsedAbAttr>(
    attrType,
    pokemon,
    (attr, passive) => attr.applyPostMoveUsed(pokemon, move, source, targets, args),
    args
  );
}

export function applyBattleStatMultiplierAbAttrs(
  attrType: { new(...args: any[]): BattleStatMultiplierAbAttr },
  pokemon: Pokemon,
  battleStat: BattleStat,
  statValue: Utils.NumberHolder,
  ...args: any[]
): Promise<void> {
  return applyAbAttrsInternal<BattleStatMultiplierAbAttr>(
    attrType,
    pokemon,
    (attr, passive) => attr.applyBattleStat(pokemon, passive, battleStat, statValue, args),
    args
  );
}
...
export function applyFieldBattleStatMultiplierAbAttrs(
  attrType: { new(...args: any[]): FieldMultiplyBattleStatAbAttr },
  pokemon: Pokemon,
  stat: Stat,
  statValue: Utils.NumberHolder,
  checkedPokemon: Pokemon,
  ...
  return applyAbAttrsInternal<FieldMultiplyBattleStatAbAttr>(
    attrType,
    pokemon,
    (attr, passive) =>
      attr.applyFieldBattleStat(
        pokemon,
        passive,
        stat,
        statValue,
...
export function applyPreAttackAbAttrs(
  attrType: { new(...args: any[]): PreAttackAbAttr },
  pokemon: Pokemon,
  defender: Pokemon,
  move: Move,
  ...args: any[]
  ...
  return applyAbAttrsInternal<PreAttackAbAttr>(
    attrType,
    pokemon,
    (attr, passive) => attr.applyPreAttack(pokemon, passive, defender, move, args),
    args
  );
}

export function applyPostAttackAbAttrs(
  attrType: { new(...args: any[]): PostAttackAbAttr },
  pokemon: Pokemon,
  defender: Pokemon,
  move: Move,
  hitResult: HitResult,
  ...
  return applyAbAttrsInternal<PostAttackAbAttr>(
    attrType,
    pokemon,
    (attr, passive) => attr.applyPostAttack(pokemon, passive, defender, move, hitResult, args),
    args
  );
}

export function applyPostKnockOutAbAttrs(
  attrType: { new(...args: any[]): PostKnockOutAbAttr },
  pokemon: Pokemon,
  knockedOut: Pokemon,
  ...args: any[]
): Promise<void> {
  return applyAbAttrsInternal<PostKnockOutAbAttr>(
    attrType,
    pokemon,
    (attr, passive) => attr.applyPostKnockOut(pokemon, passive, knockedOut, args),
    args
  );
}

export function applyPostVictoryAbAttrs(
  attrType: { new(...args: any[]): PostVictoryAbAttr },
  pokemon: Pokemon,
  ...args: any[]
): Promise<void> {
  return applyAbAttrsInternal<PostVictoryAbAttr>(
    attrType,
    pokemon,
    (attr, passive) => attr.applyPostVictory(pokemon, passive, args),
    args
  );
}

export function applyPostSummonAbAttrs(
  attrType: { new(...args: any[]): PostSummonAbAttr },
  pokemon: Pokemon,
  ...args: any[]
): Promise<void> {
  return applyAbAttrsInternal<PostSummonAbAttr>(
    attrType,
    pokemon,
    (attr, passive) => attr.applyPostSummon(pokemon, passive, args),
    args
  );
}

export function applyPreSwitchOutAbAttrs(
  attrType: { new(...args: any[]): PreSwitchOutAbAttr },
  pokemon: Pokemon,
  ...args: any[]
): Promise<void> {
  return applyAbAttrsInternal<PreSwitchOutAbAttr>(
    attrType,
    pokemon,
    (attr, passive) => attr.applyPreSwitchOut(pokemon, passive, args),
    args,
    false,
    true
  );
...
export function applyPreStatChangeAbAttrs(
  attrType: { new(...args: any[]): PreStatChangeAbAttr },
  pokemon: Pokemon,
  stat: BattleStat,
  cancelled: Utils.BooleanHolder,
  ...args: any[]
): Promise<void> {
  return applyAbAttrsInternal<PreStatChangeAbAttr>(
    attrType,
    pokemon,
    (attr, passive) => attr.applyPreStatChange(pokemon, passive, stat, cancelled, args),
    args
  );
}

export function applyPostStatChangeAbAttrs(
  attrType: { new(...args: any[]): PostStatChangeAbAttr },
  pokemon: Pokemon,
  stats: BattleStat[],
  levels: integer,
  selfTarget: boolean,
  ...
  return applyAbAttrsInternal<PostStatChangeAbAttr>(
    attrType,
    pokemon,
    (attr, passive) => attr.applyPostStatChange(pokemon, stats, levels, selfTarget, args),
    args
  );
}

export function applyPreSetStatusAbAttrs(
  attrType: { new(...args: any[]): PreSetStatusAbAttr },
  pokemon: Pokemon,
  effect: StatusEffect,
  cancelled: Utils.BooleanHolder,
  ...args: any[]
  ...
  return applyAbAttrsInternal<PreSetStatusAbAttr>(
    attrType,
    pokemon,
    (attr, passive) => attr.applyPreSetStatus(pokemon, passive, effect, cancelled, args),
    args,
    false,
    false,
    !simulated
...
export function applyPreApplyBattlerTagAbAttrs(
  attrType: { new(...args: any[]): PreApplyBattlerTagAbAttr },
  pokemon: Pokemon,
  tag: BattlerTag,
  cancelled: Utils.BooleanHolder,
  ...args: any[]
  ...
  return applyAbAttrsInternal<PreApplyBattlerTagAbAttr>(
    attrType,
    pokemon,
    (attr, passive) => attr.applyPreApplyBattlerTag(pokemon, passive, tag, cancelled, args),
    args
  );
}

export function applyPreWeatherEffectAbAttrs(
  attrType: { new(...args: any[]): PreWeatherEffectAbAttr },
  pokemon: Pokemon,
  weather: Weather,
  cancelled: Utils.BooleanHolder,
  ...args: any[]
  ...
  return applyAbAttrsInternal<PreWeatherDamageAbAttr>(
    attrType,
    pokemon,
    (attr, passive) => attr.applyPreWeatherEffect(pokemon, passive, weather, cancelled, args),
    args,
    false,
    true
  );
...
export function applyPostTurnAbAttrs(
  attrType: { new(...args: any[]): PostTurnAbAttr },
  pokemon: Pokemon,
  ...args: any[]
): Promise<void> {
  return applyAbAttrsInternal<PostTurnAbAttr>(
    attrType,
    pokemon,
    (attr, passive) => attr.applyPostTurn(pokemon, passive, args),
    args
  );
}

export function applyPostWeatherChangeAbAttrs(
  attrType: { new(...args: any[]): PostWeatherChangeAbAttr },
  pokemon: Pokemon,
  weather: WeatherType,
  ...args: any[]
): Promise<void> {
  return applyAbAttrsInternal<PostWeatherChangeAbAttr>(
    attrType,
    pokemon,
    (attr, passive) => attr.applyPostWeatherChange(pokemon, passive, weather, args),
    args
  );
}

export function applyPostWeatherLapseAbAttrs(
  attrType: { new(...args: any[]): PostWeatherLapseAbAttr },
  pokemon: Pokemon,
  weather: Weather,
  ...args: any[]
): Promise<void> {
  return applyAbAttrsInternal<PostWeatherLapseAbAttr>(
    attrType,
    pokemon,
    (attr, passive) => attr.applyPostWeatherLapse(pokemon, passive, weather, args),
    args
  );
}

export function applyPostTerrainChangeAbAttrs(
  attrType: { new(...args: any[]): PostTerrainChangeAbAttr },
  pokemon: Pokemon,
  terrain: TerrainType,
  ...args: any[]
): Promise<void> {
  return applyAbAttrsInternal<PostTerrainChangeAbAttr>(
    attrType,
    pokemon,
    (attr, passive) => attr.applyPostTerrainChange(pokemon, passive, terrain, args),
    args
  );
}

export function applyCheckTrappedAbAttrs(
  attrType: { new(...args: any[]): CheckTrappedAbAttr },
  pokemon: Pokemon,
  trapped: Utils.BooleanHolder,
  otherPokemon: Pokemon,
  ...args: any[]
  ...
  return applyAbAttrsInternal<CheckTrappedAbAttr>(
    attrType,
    pokemon,
    (attr, passive) => attr.applyCheckTrapped(pokemon, passive, trapped, otherPokemon, args),
    args,
    true
  );
}
...
export function applyPostBattleAbAttrs(
  ...
  return applyAbAttrsInternal<PostBattleAbAttr>(
    attrType,
    pokemon,
    (attr, passive) => attr.applyPostBattle(pokemon, passive, args),
    args
  );
}

export function applyPostFaintAbAttrs(
  attrType: { new(...args: any[]): PostFaintAbAttr },
  pokemon: Pokemon,
  attacker: Pokemon,
  move: Move,
  hitResult: HitResult,
  ...
  return applyAbAttrsInternal<PostFaintAbAttr>(
    attrType,
    pokemon,
    (attr, passive) => attr.applyPostFaint(pokemon, passive, attacker, move, hitResult, args),
    args
  );
}
...
export function initAbilities() {
  allAbilities.push(
    new Ability(Abilities.STENCH, 3).attr(
      ...
      false,
      (user, target, move) =>
        move.category !== MoveCategory.STATUS && !move.hasAttr(FlinchAttr) ? 10 : 0,
      BattlerTagType.FLINCHED
    ),
    new Ability(Abilities.DRIZZLE, 3)
      .attr(PostSummonWeatherChangeAbAttr, WeatherType.RAIN)
      .attr(PostBiomeChangeWeatherChangeAbAttr, WeatherType.RAIN),
    new Ability(Abilities.SPEED_BOOST, 3).attr(PostTurnStatChangeAbAttr, BattleStat.SPD, 1),
    ...
    new Ability(Abilities.OBLIVIOUS, 3)
      ...
      .ignorable(),
    new Ability(Abilities.CLOUD_NINE, 3).attr(SuppressWeatherEffectAbAttr, true),
    new Ability(Abilities.COMPOUND_EYES, 3).attr(BattleStatMultiplierAbAttr, BattleStat.ACC, 1.3),
    new Ability(Abilities.INSOMNIA, 3)
      .attr(StatusEffectImmunityAbAttr, StatusEffect.SLEEP)
      .attr(BattlerTagImmunityAbAttr, BattlerTagType.DROWSY)
      .ignorable(),
    new Ability(Abilities.COLOR_CHANGE, 3).attr(PostDefendTypeChangeAbAttr),
    ...
    new Ability(Abilities.FLASH_FIRE, 3)
      .attr(
        ...
        1,
        (pokemon: Pokemon) => !pokemon.status || pokemon.status.effect !== StatusEffect.FREEZE
      )
      .ignorable(),
    new Ability(Abilities.SHIELD_DUST, 3).ignorable().unimplemented(),
    ...
    new Ability(Abilities.LEVITATE, 3)
      .attr(
        ...
        Type.GROUND,
        (pokemon: Pokemon) =>
          !pokemon.getTag(BattlerTagType.IGNORE_FLYING) &&
          !pokemon.scene.arena.getTag(ArenaTagType.GRAVITY) &&
          !pokemon.getTag(BattlerTagType.GROUNDED)
      )
      .ignorable(),
    new Ability(Abilities.EFFECT_SPORE, 3).attr(EffectSporeAbAttr),
    new Ability(Abilities.SYNCHRONIZE, 3).attr(SyncEncounterNatureAbAttr).unimplemented(),
    ...
    new Ability(Abilities.LIGHTNING_ROD, 3)
      ...
      .ignorable(),
    new Ability(Abilities.SERENE_GRACE, 3).unimplemented(),
    ...
    new Ability(Abilities.ILLUMINATE, 3)
      ...
      .ignorable(),
    new Ability(Abilities.TRACE, 3).attr(TraceAbAttr).attr(UncopiableAbilityAbAttr),
    new Ability(Abilities.HUGE_POWER, 3).attr(BattleStatMultiplierAbAttr, BattleStat.ATK, 2),
    ...
    new Ability(Abilities.PRESSURE, 3)
      ...
      .attr(PostSummonMessageAbAttr, (pokemon: Pokemon) =>
        getPokemonMessage(pokemon, " is exerting its Pressure!")
      ),
    ...
    new Ability(Abilities.KEEN_EYE, 3).attr(ProtectStatAbAttr, BattleStat.ACC).ignorable(),
    new Ability(Abilities.HYPER_CUTTER, 3).attr(ProtectStatAbAttr, BattleStat.ATK).ignorable(),
    ...
    new Ability(Abilities.HUSTLE, 3)
      .attr(
        ...
        BattleStat.ATK,
        1.5,
        (user, target, move) => move.category === MoveCategory.PHYSICAL
      )
      .attr(
        BattleStatMultiplierAbAttr,
        BattleStat.ACC,
    ...
    new Ability(Abilities.PLUS, 3)
      .conditionalAttr(
        (p) =>
          p.scene.currentBattle.double &&
          [Abilities.PLUS, Abilities.MINUS].some((a) => p.getAlly().hasAbility(a)),
        BattleStatMultiplierAbAttr,
        BattleStat.SPATK,
        1.5
      )
    ...
    new Ability(Abilities.PURE_POWER, 3).attr(BattleStatMultiplierAbAttr, BattleStat.ATK, 2),
    ...
    new Ability(Abilities.STEADFAST, 4).attr(FlinchStatChangeAbAttr, BattleStat.SPD, 1),
    ...
    new Ability(Abilities.ANGER_POINT, 4).attr(PostDefendCritStatChangeAbAttr, BattleStat.ATK, 6),
    new Ability(Abilities.UNBURDEN, 4).unimplemented(),
    new Ability(Abilities.HEATPROOF, 4)
      .attr(ReceivedTypeDamageMultiplierAbAttr, Type.FIRE, 0.5)
      .ignorable(),
    new Ability(Abilities.SIMPLE, 4).attr(StatChangeMultiplierAbAttr, 2).ignorable(),
    ...
    new Ability(Abilities.STALL, 4).unimplemented(),
    ...
    new Ability(Abilities.MOLD_BREAKER, 4)
      .attr(PostSummonMessageAbAttr, (pokemon: Pokemon) =>
        getPokemonMessage(pokemon, " breaks the mold!")
      )
      .attr(MoveAbilityBypassAbAttr),
    new Ability(Abilities.SUPER_LUCK, 4).attr(BonusCritAbAttr).partial(),
    ...
    new Ability(Abilities.ANTICIPATION, 4).conditionalAttr(
      ...
      (pokemon: Pokemon) => getPokemonMessage(pokemon, " shuddered!")
    ),
    new Ability(Abilities.FOREWARN, 4).attr(ForewarnAbAttr),
    new Ability(Abilities.UNAWARE, 4).attr(IgnoreOpponentStatChangesAbAttr).ignorable(),
    new Ability(Abilities.TINTED_LENS, 4).attr(
      DamageBoostAbAttr,
      2,
      (user, target, move) => target.getAttackTypeEffectiveness(move.type, user) <= 0.5
    ),
    new Ability(Abilities.FILTER, 4)
      .attr(
        ReceivedMoveDamageMultiplierAbAttr,
        (target, user, move) => target.getAttackTypeEffectiveness(move.type, user) >= 2,
        0.75
      )
      .ignorable(),
    ...
    new Ability(Abilities.PICKPOCKET, 5).attr(PostDefendStealHeldItemAbAttr, (target, user, move) =>
      move.hasFlag(MoveFlags.MAKES_CONTACT)
    ),
    new Ability(Abilities.SHEER_FORCE, 5).unimplemented(),
    new Ability(Abilities.CONTRARY, 5).attr(StatChangeMultiplierAbAttr, -1).ignorable(),
    ...
    new Ability(Abilities.DEFIANT, 5).attr(
      ...
      (target, statsChanged, levels) => levels < 0,
      [BattleStat.ATK],
      2
    ),
    ...
    new Ability(Abilities.FRIEND_GUARD, 5).ignorable().unimplemented(),
    new Ability(Abilities.WEAK_ARMOR, 5)
      .attr(
        PostDefendStatChangeAbAttr,
    ...
    new Ability(Abilities.MULTISCALE, 5)
      .attr(
        ReceivedMoveDamageMultiplierAbAttr,
        (target, user, move) => target.getHpRatio() === 1,
        0.5
      )
      .ignorable(),
    ...
    new Ability(Abilities.FLARE_BOOST, 5).attr(
      MovePowerBoostAbAttr,
      (user, target, move) =>
        move.category === MoveCategory.SPECIAL && user.status.effect === StatusEffect.BURN,
      1.5
    ),
    new Ability(Abilities.HARVEST, 5)
      .attr(
        ...
        (pokemon) =>
          0.5 * (getWeatherCondition(WeatherType.SUNNY, WeatherType.HARSH_SUN)(pokemon) ? 2 : 1)
      )
      .partial(),
    ...
    new Ability(Abilities.BIG_PECKS, 5).attr(ProtectStatAbAttr, BattleStat.DEF).ignorable(),
    ...
    new Ability(Abilities.IMPOSTER, 5)
      ...
      .attr(UncopiableAbilityAbAttr),
    new Ability(Abilities.INFILTRATOR, 5).unimplemented(),
    new Ability(Abilities.MUMMY, 5)
      .attr(PostDefendAbilityGiveAbAttr, Abilities.MUMMY)
      .bypassFaint(),
    new Ability(Abilities.MOXIE, 5).attr(PostVictoryStatChangeAbAttr, BattleStat.ATK, 1),
    new Ability(Abilities.JUSTIFIED, 5).attr(
      PostDefendStatChangeAbAttr,
      (target, user, move) => move.type === Type.DARK && move.category !== MoveCategory.STATUS,
      BattleStat.ATK,
      1
    ),
    new Ability(Abilities.RATTLED, 5)
      .attr(
        ...
        (target, user, move) =>
          ...
          (move.type === Type.DARK || move.type === Type.BUG || move.type === Type.GHOST),
        BattleStat.SPD,
        1
      )
    ...
    new Ability(Abilities.TURBOBLAZE, 5)
      .attr(PostSummonMessageAbAttr, (pokemon: Pokemon) =>
        getPokemonMessage(pokemon, " is radiating a blazing aura!")
      )
      .attr(MoveAbilityBypassAbAttr),
    new Ability(Abilities.TERAVOLT, 5)
      .attr(PostSummonMessageAbAttr, (pokemon: Pokemon) =>
        getPokemonMessage(pokemon, " is radiating a bursting aura!")
      )
      .attr(MoveAbilityBypassAbAttr),
    new Ability(Abilities.AROMA_VEIL, 6).ignorable().unimplemented(),
    new Ability(Abilities.FLOWER_VEIL, 6).ignorable().unimplemented(),
    new Ability(Abilities.CHEEK_POUCH, 6).attr(HealFromBerryUseAbAttr, 1 / 3).partial(), // Healing not blocked by Heal Block
    ...
    new Ability(Abilities.REFRIGERATE, 6).attr(
      ...
      Type.ICE,
      1.2,
      (user, target, move) => move.type === Type.NORMAL
    ),
    ...
    new Ability(Abilities.GALE_WINGS, 6).attr(
      IncrementMovePriorityAbAttr,
      (pokemon, move) => pokemon.getHpRatio() === 1 && move.type === Type.FLYING
    ),
    ...
    new Ability(Abilities.PARENTAL_BOND, 6).unimplemented(),
    new Ability(Abilities.DARK_AURA, 6)
      .attr(PostSummonMessageAbAttr, (pokemon: Pokemon) =>
        getPokemonMessage(pokemon, " is radiating a Dark Aura!")
      )
      .attr(FieldMoveTypePowerBoostAbAttr, Type.DARK, 4 / 3),
    new Ability(Abilities.FAIRY_AURA, 6)
      .attr(PostSummonMessageAbAttr, (pokemon: Pokemon) =>
        getPokemonMessage(pokemon, " is radiating a Fairy Aura!")
      )
      .attr(FieldMoveTypePowerBoostAbAttr, Type.FAIRY, 4 / 3),
    new Ability(Abilities.AURA_BREAK, 6).ignorable().unimplemented(),
    ...
    new Ability(Abilities.WIMP_OUT, 7).unimplemented(),
    new Ability(Abilities.EMERGENCY_EXIT, 7).unimplemented(),
    new Ability(Abilities.WATER_COMPACTION, 7).attr(
      PostDefendStatChangeAbAttr,
      (target, user, move) => move.type === Type.WATER && move.category !== MoveCategory.STATUS,
      BattleStat.DEF,
      2
    ),
    ...
    new Ability(Abilities.STAKEOUT, 7).attr(
      MovePowerBoostAbAttr,
      (user, target, move) =>
        user.scene.currentBattle.turnCommands[target.getBattlerIndex()]?.command === Command.POKEMON,
      2
    ),
    ...
    new Ability(Abilities.BERSERK, 7).attr(
      PostDefendHpGatedStatChangeAbAttr,
      (target, user, move) => move.category !== MoveCategory.STATUS,
      0.5,
      [BattleStat.SPATK],
      1
    ...
    new Ability(Abilities.DISGUISE, 7)
      .attr(
        PreDefendMovePowerToOneAbAttr,
        (target, user, move) =>
          target.formIndex === 0 && target.getAttackTypeEffectiveness(move.type, user) > 0
      )
    ...
    new Ability(Abilities.POWER_CONSTRUCT, 7) // TODO: 10% Power Construct Zygarde isn't accounted for yet. If changed, update Zygarde's getSpeciesFormIndex entry accordingly
      ...
      .attr(PostSummonFormChangeAbAttr, (p) =>
        p.getHpRatio() <= 0.5 || p.getFormKey() === "complete" ? 4 : 2
      )
    ...
    new Ability(Abilities.BATTERY, 7).unimplemented(),
    ...
    new Ability(Abilities.SOUL_HEART, 7).attr(PostKnockOutStatChangeAbAttr, BattleStat.SPATK, 1),
    ...
    new Ability(Abilities.BEAST_BOOST, 7).attr(
      PostVictoryStatChangeAbAttr,
      (p) => {
        const battleStats = Object.values(BattleStat)
          .slice(0, -3)
          .map((s) => s as BattleStat);
    ...
    new Ability(Abilities.NEUROFORCE, 7).attr(
      MovePowerBoostAbAttr,
      (user, target, move) => target.getAttackTypeEffectiveness(move.type, user) >= 2,
      1.25
    ),
    ...
    new Ability(Abilities.STEAM_ENGINE, 8).attr(
      PostDefendStatChangeAbAttr,
      (target, user, move) =>
        (move.type === Type.FIRE || move.type === Type.WATER) &&
        move.category !== MoveCategory.STATUS,
      BattleStat.SPD,
      6
    ),
    new Ability(Abilities.PUNK_ROCK, 8)
      ...
      .ignorable(),
    new Ability(Abilities.SAND_SPIT, 8).attr(PostDefendWeatherChangeAbAttr, WeatherType.SANDSTORM),
    ...
    new Ability(Abilities.ICE_FACE, 8)
      ...
      .attr(
        IceFaceMoveImmunityAbAttr,
        (target, user, move) =>
          move.category === MoveCategory.PHYSICAL && !!target.getTag(BattlerTagType.ICE_FACE)
      )
      .ignorable(),
    new Ability(Abilities.POWER_SPOT, 8).unimplemented(),
    new Ability(Abilities.MIMICRY, 8).unimplemented(),
    new Ability(Abilities.SCREEN_CLEANER, 8).unimplemented(),
    new Ability(Abilities.STEELY_SPIRIT, 8).attr(MoveTypePowerBoostAbAttr, Type.STEEL).partial(),
    new Ability(Abilities.PERISH_BODY, 8).attr(PostDefendPerishSongAbAttr, 4),
    new Ability(Abilities.WANDERING_SPIRIT, 8)
      .attr(PostDefendAbilitySwapAbAttr)
      .bypassFaint()
      .partial(),
    new Ability(Abilities.GORILLA_TACTICS, 8).unimplemented(),
    new Ability(Abilities.NEUTRALIZING_GAS, 8)
      ...
      .attr(NoTransformAbilityAbAttr)
      .attr(PostSummonMessageAbAttr, (pokemon: Pokemon) =>
        getPokemonMessage(pokemon, "'s Neutralizing Gas filled the area!")
      )
      .partial(),
    ...
    new Ability(Abilities.QUICK_DRAW, 8).unimplemented(),
    new Ability(Abilities.UNSEEN_FIST, 8).attr(IgnoreProtectOnContactAbAttr),
    new Ability(Abilities.CURIOUS_MEDICINE, 8).attr(PostSummonClearAllyStatsAbAttr),
    ...
    new Ability(Abilities.CHILLING_NEIGH, 8).attr(PostVictoryStatChangeAbAttr, BattleStat.ATK, 1),
    new Ability(Abilities.GRIM_NEIGH, 8).attr(PostVictoryStatChangeAbAttr, BattleStat.SPATK, 1),
    new Ability(Abilities.AS_ONE_GLASTRIER, 8)
      .attr(PostSummonMessageAbAttr, (pokemon: Pokemon) =>
        getPokemonMessage(pokemon, " has two Abilities!")
      )
    ...
    new Ability(Abilities.THERMAL_EXCHANGE, 9)
      .attr(
        PostDefendStatChangeAbAttr,
        (target, user, move) => move.type === Type.FIRE && move.category !== MoveCategory.STATUS,
        BattleStat.ATK,
        1
      )
      .attr(StatusEffectImmunityAbAttr, StatusEffect.BURN)
    ...
    new Ability(Abilities.VESSEL_OF_RUIN, 9)
      ...
      .attr(PostSummonMessageAbAttr, (user) =>
        getPokemonMessage(
          user,
          `'s Vessel of Ruin lowered the ${getStatName(Stat.SPATK)}\nof all surrounding Pokémon!`
        )
      )
      .ignorable(),
    new Ability(Abilities.SWORD_OF_RUIN, 9)
      ...
      .attr(PostSummonMessageAbAttr, (user) =>
        getPokemonMessage(
          user,
          `'s Sword of Ruin lowered the ${getStatName(Stat.DEF)}\nof all surrounding Pokémon!`
        )
      )
      .ignorable(),
    new Ability(Abilities.TABLETS_OF_RUIN, 9)
      ...
      .attr(PostSummonMessageAbAttr, (user) =>
        getPokemonMessage(
          user,
          `'s Tablets of Ruin lowered the ${getStatName(Stat.ATK)}\nof all surrounding Pokémon!`
        )
      )
      .ignorable(),
    new Ability(Abilities.BEADS_OF_RUIN, 9)
      ...
      .attr(PostSummonMessageAbAttr, (user) =>
        getPokemonMessage(
          user,
          `'s Beads of Ruin lowered the ${getStatName(Stat.SPDEF)}\nof all surrounding Pokémon!`
        )
      )
      .ignorable(),
    ...
    new Ability(Abilities.OPPORTUNIST, 9).attr(StatChangeCopyAbAttr),
    ...
    new Ability(Abilities.SUPREME_OVERLORD, 9)
      ...
      .partial(),
    new Ability(Abilities.COSTAR, 9).unimplemented(),
    ...
    new Ability(Abilities.HOSPITALITY, 9).attr(PostSummonAllyHealAbAttr, 4, true).partial(), // Healing not blocked by Heal Block
    ...
    new Ability(Abilities.POISON_PUPPETEER, 9)
      ...
      .attr(UnswappableAbilityAbAttr)
      .conditionalAttr(
        (pokemon) => pokemon.species.speciesId === Species.PECHARUNT,
