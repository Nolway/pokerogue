...
type TrainerNameConfigs = Record<integer, TrainerNameConfig>;
...
const trainerNameConfigs: TrainerNameConfigs = {
  ...
  [TrainerType.BLACK_BELT]: new TrainerNameConfig(TrainerType.BLACK_BELT).hasGenderVariant(
    "Battle_Girl"
  ),
  ...
  [TrainerType.RICH]: new TrainerNameConfig(TrainerType.RICH, "Gentleman").hasGenderVariant(
    "Madame"
  ),
  [TrainerType.RICH_KID]: new TrainerNameConfig(TrainerType.RICH_KID, "Rich_Boy").hasGenderVariant(
    "Lady"
  ),
  ...
  return new Promise<void>((resolve) => {
    fetch(`https://bulbapedia.bulbagarden.net/wiki/${url}_(Trainer_class)`)
      ...
      .then((html) => {
        ...
        const endChildIndex = elements.findIndex(
          (h) => h.nodeName === "H2" && elements.indexOf(h) > startChildIndex
        );
        ...
        for (const table of tables) {
          ...
          for (const row of trainerRows) {
            ...
            if (content.indexOf(" <a ") > -1) {
              ...
              if (nameMatch) {
                (female || forceFemale ? femaleTrainerNames : trainerNames).add(
                  nameMatch[1].replace("&amp;", "&")
                );
              }
            }
