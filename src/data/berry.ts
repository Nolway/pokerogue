...
export function getBerryPredicate(berryType: BerryType): BerryPredicate {
  ...
  case BerryType.ENIGMA:
    return (pokemon: Pokemon) =>
      !!pokemon.turnData.attacksReceived.filter((a) => a.result === HitResult.SUPER_EFFECTIVE)
        .length;
  ...
  case BerryType.SALAC:
    return (pokemon: Pokemon) => {
      ...
      return (
        pokemon.getHpRatio() < threshold.value && pokemon.summonData.battleStats[battleStat] < 6
      );
    };
    ...
    return (pokemon: Pokemon) => {
      ...
      pokemon.scene.unshiftPhase(
        new PokemonHealPhase(
          ...
          hpHealed.value,
          getPokemonMessage(pokemon, `'s ${getBerryName(berryType)}\nrestored its HP!`),
          true
        )
      );
    };
    ...
    return (pokemon: Pokemon) => {
      ...
      if (ppRestoreMove !== undefined) {
        ...
        pokemon.scene.queueMessage(
          getPokemonMessage(
            pokemon,
            ` restored PP to its move ${ppRestoreMove.getName()}\nusing its ${getBerryName(berryType)}!`
          )
        );
      }
    };
