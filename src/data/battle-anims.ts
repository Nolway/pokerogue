    ...
    if (moveAnims.has(move)) {
      if (moveAnims.get(move) !== null) {
        ...
        const loadedCheckTimer = setInterval(() => {
          if (moveAnims.get(move) !== null) {
            const chargeAttr =
              allMoves[move].getAttrs(ChargeAttr)[0] ||
      ...
      const fetchAnimAndResolve = (move: Moves) => {
        scene
          .cachedFetch(`./battle-anims/${moveName}.json`)
          .then((response) => {
            if (!response.ok) {
              console.error(
...
export function loadMoveAnimAssets(
  ...
  return new Promise((resolve) => {
    ...
    for (const moveId of moveIds) {
      const chargeAttr =
        allMoves[moveId].getAttrs(ChargeAttr)[0] || allMoves[moveId].getAttrs(DelayedAttackAttr)[0];
      if (chargeAttr) {
        const moveChargeAnims = chargeAnims.get(chargeAttr.chargeAnim);
        moveAnimations.push(
          moveChargeAnims instanceof AnimConfig ? moveChargeAnims : moveChargeAnims[0]
        );
...
type SpriteCache = Record<integer, Phaser.GameObjects.Sprite[]>;
...
export abstract class BattleAnim {
  ...
  private getGraphicFrameData(
    ...
    const ret = new Map<integer, Map<AnimFrameTarget, GraphicFrameData>>([
          ...
          if (frame.target !== AnimFrameTarget.GRAPHIC) {
            ...
            if ((isUser ? u : t) === sprites.length) {
              ...
              ["spriteColors", "fusionSpriteColors"].map(
                (k) =>
                  (sprite.pipelineData[k] = (isUser ? user : target).getSprite().pipelineData[k])
              );
            ...
            pokemonSprite.setPosition(
              graphicFrameData.x,
              graphicFrameData.y -
                (spriteSource.height / 2) * (spriteSource.parentContainer.scale - 1)
            );

            pokemonSprite.setAngle(graphicFrameData.angle);
            pokemonSprite.setScale(
              graphicFrameData.scaleX * spriteSource.parentContainer.scale,
              graphicFrameData.scaleY * spriteSource.parentContainer.scale
            );
            ...
            if (g === sprites.length) {
              const newSprite: Phaser.GameObjects.Sprite = scene.addFieldSprite(
                0,
                0,
                anim.graphic,
                1
              ...
              const setSpritePriority = (priority: integer) => {
                ...
                case 0:
                  scene.field.moveBelow(
                      ...
                      scene.getEnemyPokemon() || scene.getPlayerPokemon()
                  );
                  break;
                case 1:
...
export async function populateAnims() {
  ...
  const commonAnimIds = Object.values(CommonAnim) as CommonAnim[];
  const chargeAnimNames = Object.keys(ChargeAnim).map((k) => k.toLowerCase());
  const chargeAnimMatchNames = chargeAnimNames.map((k) => k.replace(/\_/g, " "));
  const chargeAnimIds = Object.values(ChargeAnim) as ChargeAnim[];
  const commonNamePattern = /name: (?:Common:)?(Opp )?(.*)/;
  const moveNameToId = {};
  for (const move of Object.values(Moves).slice(1)) {
  ...
  for (let a = 0; a < animsData.length; a++) {
    ...
    if (commonAnimId) {
      ...
      moveAnims.set(
        ...
        !isOppMove ? anim : [moveAnims.get(moveNameToId[animName]) as AnimConfig, anim]
      );
    }
    for (let f = 0; f < fields.length; f++) {
      ...
      case "graphic":
        ...
        anim.graphic =
            graphic.indexOf(".") > -1 ? graphic.slice(0, fieldData.indexOf(".")) : graphic;
        break;
      case "timing":
