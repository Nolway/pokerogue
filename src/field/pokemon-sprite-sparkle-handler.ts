...
export default class PokemonSpriteSparkleHandler {
  ...
  onLapse(): void {
    ...
    for (const s of this.sprites.values()) {
      if (
        !s.pipelineData["teraColor"] ||
        !(s.pipelineData["teraColor"] as number[]).find((c) => c)
      ) {
        continue;
      }
      if (
        !s.visible ||
        (s.parentContainer instanceof Pokemon && !s.parentContainer.parentContainer)
      ) {
        continue;
      }
      const pokemon = s.parentContainer instanceof Pokemon ? s.parentContainer : null;
      ...
      if (pixel.alpha) {
        ...
        const sparkle = (s.scene as BattleScene).addFieldSprite(
        ...
        parent.add(sparkle);
        s.scene.time.delayedCall(Utils.fixedInt(Math.floor((1000 / 12) * 13)), () =>
          sparkle.destroy()
        );
      }
    }
