...
export type DeviceMapping = Record<string, number>;

export type IconsMapping = Record<string, string>;

export type SettingMapping = Record<string, Button>;

export type MappingLayout = Record<string, SettingGamepad | SettingKeyboard | number>;
...
export class InputsController {
  private gamepads = new Array<Phaser.Input.Gamepad.Gamepad>();
  private scene: BattleScene;
  public events: Phaser.Events.EventEmitter;

  private buttonLock: Button[] = [];
  private interactions = new Map<Button, Map<string, boolean>>();
  private configs = new Map<string, InterfaceConfig>();

  public gamepadSupport: boolean = true;
  public selectedDevice;

  private disconnectedGamepads = new Array<string>();
  ...
  setupGamepad(thisGamepad: Phaser.Input.Gamepad.Gamepad): void {
    ...
    for (const gamepad of allGamepads) {
      ...
      const config = { ...this.getConfig(gamepadID) } satisfies InterfaceConfig;
  ...
  setupKeyboard(): void {
    for (const layout of ["default"]) {
      ...
      this.scene.gameData.saveMappingConfigs(
        this.selectedDevice[Device.KEYBOARD],
        this.configs[layout]
      );
    }
  ...
  gamepadButtonDown(
    ...
    if (
      !this.gamepadSupport ||
      pad.id.toLowerCase() !== this.selectedDevice[Device.GAMEPAD].toLowerCase()
    ) {
      return;
    }
