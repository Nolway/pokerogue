...
export function addWindow(
  ...
  if (mergeMaskTop || mergeMaskLeft) {
    ...
    maskRect.beginPath();
    maskRect.fillRect(
      window.x + (mergeMaskLeft ? 2 : 0) + (maskOffsetX || 0),
      window.y + (mergeMaskTop ? 2 : 0) + (maskOffsetY || 0),
      window.width - (mergeMaskLeft ? 2 : 0),
      window.height - (mergeMaskTop ? 2 : 0)
    );
...
export function updateWindowType(scene: BattleScene, windowTypeIndex: integer): void {
  ...
  const traverse = (object: any) => {
    if (
      ...
      object.children instanceof Phaser.GameObjects.DisplayList
    ) {
  ...
  ["base", "light", "dark"].map((k, i) =>
    rootStyle.setProperty(
      `--color-${k}`,
      windowTypeControlColors[scene.uiTheme][windowTypeIndex - 1][i]
    )
  );
