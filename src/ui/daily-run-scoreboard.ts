...
export class DailyRunScoreboard extends Phaser.GameObjects.Container {
  ...
  setup() {
    ...
    this.prevCategoryButton.setInteractive(
      new Phaser.Geom.Rectangle(0, 0, 6, 10),
      Phaser.Geom.Rectangle.Contains
    );
    this.prevCategoryButton.on("pointerup", () => {
      this.update(
        this.category ? this.category - 1 : Object.keys(ScoreboardCategory).length - 1
      );
    });
    ...
    this.nextCategoryButton.setInteractive(
      new Phaser.Geom.Rectangle(0, 0, 6, 10),
      Phaser.Geom.Rectangle.Contains
    );
    this.nextCategoryButton.on("pointerup", () => {
      this.update(
        this.category < Object.keys(ScoreboardCategory).length - 1 ? this.category + 1 : 0
      );
    });
  ...
  updateRankings(rankings: RankingEntry[]) {
    const getEntry = (rank: string, username: string, score: string, wave: string) => {
      ...
      const rankLabel = addTextObject(this.scene, 0, 0, rank, TextStyle.WINDOW, {
        fontSize: "54px"
      });
      entryContainer.add(rankLabel);

      const usernameLabel = addTextObject(this.scene, 12, 0, username, TextStyle.WINDOW, {
        fontSize: "54px"
      });
      entryContainer.add(usernameLabel);

      const scoreLabel = addTextObject(this.scene, 84, 0, score, TextStyle.WINDOW, {
        fontSize: "54px"
      });
      ...
      case ScoreboardCategory.DAILY:
        const waveLabel = addTextObject(this.scene, 68, 0, wave, TextStyle.WINDOW, {
          fontSize: "54px"
        });
        ...
        Utils.apiFetch(`daily/rankings?category=${category}&page=${page}`)
          ...
          .then((jsonResponse) => {
            ...
            this.titleLabel.setText(
              `${i18next.t(`menu:${ScoreboardCategory[category].toLowerCase()}Rankings`)}`
            );
