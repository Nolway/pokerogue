...
describe("Abilities - Protean", () => {
  ...
  test(
    ...
    async () => {
      ...
      expect(
        leadPokemon.summonData.abilitiesApplied.filter((a) => a === Abilities.PROTEAN)
      ).toHaveLength(1);
  ...
  test(
    ...
    async () => {
      ...
      vi.spyOn(game.scene.getCurrentPhase() as MoveEffectPhase, "hitCheck").mockReturnValueOnce(
        false
      );
      await game.phaseInterceptor.to(TurnEndPhase);
