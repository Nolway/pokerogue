...
import { generateStarter, waitUntil } from "./gameManagerUtils";
import {
  CommandPhase,
  EncounterPhase,
  FaintPhase,
...
import PhaseInterceptor from "./phaseInterceptor";
...
export default class GameManager {
  ...
  onNextPrompt(
    ...
    this.phaseInterceptor.addToNextPrompt(
      phaseTarget,
      mode,
      callback,
      expireFn,
  ...
  doAttack(movePosition: integer) {
    this.onNextPrompt("CommandPhase", Mode.COMMAND, () => {
      this.scene.ui.setMode(
        Mode.FIGHT,
        (this.scene.getCurrentPhase() as CommandPhase).getFieldIndex()
      );
    });
